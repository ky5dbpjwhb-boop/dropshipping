...
def _fetch_bestsellers_for_keyword_impl(keyword: str, limit: int = 20) -> List[RawProduct]:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # DEBUG: Screenshot speichern, damit man sehen kann, was die
            # Seite tatsächlich zeigt (z.B. Captcha/Blockade statt Ergebnisse).
            import os
            os.makedirs("debug", exist_ok=True)
            safe_keyword = keyword.replace(" ", "_") or "leer"
            page.screenshot(path=f"debug/{safe_keyword}.png", full_page=True)
